...
(function () {
  ...
  scene('Loops over files',['What shopt does','Why nullglob matters','Matching filenames','No matching files'],(d,s)=>{
    title(d,'Loops over files');
    if(s===0){
      text(d,'shopt-meaning',640,165,'shopt: built into Bash; controls shell options',32);
      code(d,'shopt',['shopt nullglob','shopt -s nullglob','shopt -u nullglob'],110,270,34,90);
      ['Show on / off','Enable','Disable'].forEach((label,i)=>text(d,'option-effect-'+i,960,270+i*90,label,32,i===2?P.orange:P.green));
    }else if(s===1){
      text(d,'no-matches',640,180,'No matching .txt files',35);
      d.table('nullglob',85,260,[295,495,320],[['nullglob','*.txt becomes','Loop runs'],['Off (default)','literal *.txt','Once'],['On','no items','Zero times']],{rowHeight:77,fontSize:30,highlightRows:[2]});
    }else{
      code(d,'files',['shopt -s nullglob','for file in *.txt; do','    echo "Processing $file"','done'],110,205,33,63,s===3?0:1);
      text(d,'iterations',640,480,s===3?'No matches: zero iterations':'One iteration per matching file',32,s===3?P.orange:P.green);
    }
  },'shopt is a command built into Bash that displays or changes optional shell behavior; it is not a separate program to install. shopt nullglob displays whether that option is on or off. shopt -s nullglob enables it; shopt -u nullglob disables it. These settings affect the Bash shell running the command and persist until changed or that shell exits. Running a script in a separate Bash process does not change the parent shell. The -u flag here is different from set -u, which checks unset-variable expansions. Globbing expands *.txt before the loop runs. With ordinary Bash defaults, nullglob is off: if there are no matching filenames, the literal string *.txt remains, so the for loop runs once and prints Processing *.txt. With nullglob enabled, the unmatched pattern is removed and this loop runs zero times. The comparison assumes filename expansion is enabled and failglob is off, as with ordinary Bash defaults. Quote "$file" when passing the selected filename to a command, but leave the *.txt pattern unquoted so it expands. The source uses this to introduce batched processing; it does not parse the contents of the files. shopt is Bash-specific; this example belongs in a Bash script, not a Zsh prompt or a generic sh script. Ask what a filename containing a space does in this loop: it remains one loop item and the quoted expansion remains one argument.', '23',{minutes:3,sources:[BASH+'The-Shopt-Builtin.html',BASH+'Filename-Expansion.html']});
  ...
  },'This is the source appendix’s lower-level environment workflow, shown separately from the uv project workflow. uv venv creates an environment without scaffolding a complete application project. The activation command shown is for Bash/Zsh on Unix-like systems or WSL; Windows activation paths and syntax differ. Activation is optional: .venv/bin/python app.py selects the environment directly. Once active, uv pip install can install packages or read a requirements file. These commands use uv’s pip-compatible interface; they do not imply that pip is installed in the environment. uv pip install does not automatically update a project’s pyproject.toml and uv.lock. Use uv add for project-managed dependencies.', '53',{activity:'venv',minutes:2,sources:[UV+'pip/environments/',UV+'pip/packages/']});
  ...
  },'Open the virtual environment setup activity and follow its creation diagram. Identify the source file, the environment’s Python entry point, and its installed third-party packages. A new virtual environment excludes global third-party packages by default. Predict whether import pandas succeeds before installation, then add pandas and select the environment’s Python to run the script. Compare the uv project workflow (init, add, run) with creating only an environment (venv, pip install, an explicit Python path). Activation changes command lookup in the current shell; it neither creates the environment nor installs dependencies. In the real-terminal demo, inspect sys.executable rather than relying on a prompt label. Keep source files outside .venv, then compare a second fresh project to show that it does not inherit the first project’s packages. Use the separate dependency-conflict activity when discussing incompatible version ranges.', '52',{kind:'activity',activity:'venv',minutes:5,sources:[PY+'library/venv.html',UV+'pip/environments/']});
